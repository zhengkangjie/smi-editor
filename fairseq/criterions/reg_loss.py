...
@register_criterion("finetune_mse")
class FinetuneMSELoss(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        net_output = model(
            **sample["net_input"],
            features_only=True,
            classification_head_name=self.cfg.classification_head_name,
        )
        ...
        # print('sample["target"]["finetune_target"] size:', sample["target"]["finetune_target"].size())
        # if not self.training or self.fine_tune:
        if not self.training:
            ...
            logging_output = {
                "loss": loss.data,
                "predict": reg_output.view(-1, self.cfg.num_classes).data,
                "target": sample["target"]["finetune_target"]
                .view(-1, self.cfg.num_classes)
                .data,
                "smi_name": sample["smi_name"],
                "sample_size": sample_size,
                "num_task": self.cfg.num_classes,
                "conf_size": self.cfg.conf_size,
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        predicts = net_output.view(-1, self.cfg.num_classes).float()
        targets = (
            sample["target"]["finetune_target"].view(-1, self.cfg.num_classes).float()
        )
        if self.task.mean and self.task.std:
...
@register_criterion("finetune_mae")
...
@register_criterion("finetune_smooth_mae")
...
@register_criterion("finetune_mse_pocket")

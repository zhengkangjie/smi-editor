...
@register_criterion("masked_lm_with_unimol_loss", dataclass=MaskedLmConfig)
class MaskedLmWithUnimolLoss(FairseqCriterion):
    """
    ...
    def forward(self, model, sample, reduce=True, **kwargs):
        ...
        logits, encoder_distance, encoder_coord, x_norm, delta_encoder_pair_rep_norm = model(**sample["net_input"], encoder_masked_tokens=masked_tokens, **kwargs)
        ...
        if encoder_coord is not None:
            # real = mask + delta
            coord_target = sample["coord_target"]
            ...
            loss = loss + masked_coord_loss * self.cfg.masked_coord_loss 
            # restore the scale of loss for displaying
            logging_output["masked_coord_loss"] = masked_coord_loss.data * torch.sum(masked_tokens)
        ...
        if (
            self.cfg.delta_pair_repr_norm_loss > 0
            and delta_encoder_pair_rep_norm is not None
        ):
            loss = (
                loss + self.cfg.delta_pair_repr_norm_loss * delta_encoder_pair_rep_norm
            )
            logging_output[

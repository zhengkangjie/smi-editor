...
@register_criterion("finetune_cross_entropy")
class FinetuneCrossEntropyLoss(CrossEntropyCriterion):
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        if "valid" in split or "test" in split:
            ...
            if probs.size(-1) == 2:
                ...
                # print('df["targets"]:', df["targets"])
                # print('df["probs"]:', df["probs"])
...
@register_criterion("multi_task_BCE")
class MultiTaskBCELoss(CrossEntropyCriterion):
    def __init__(self, cfg, task):
        super().__init__(cfg, task)
        self.cfg = cfg
...
@register_criterion("finetune_cross_entropy_pocket")

...
class Trainer(object):
    ...
    def valid_step(self, sample, raise_oom=False, subset='valid'):
    ...
    def _aggregate_logging_outputs(
        ...
        *extra_stats_to_sum,
        ...
        function_signature = inspect.signature(self.task.__class__.logging_outputs_can_be_summed)
        if 'is_train' in function_signature.parameters:
            can_be_summed = self.task.__class__.logging_outputs_can_be_summed(self.get_criterion(), is_train=is_train)
        else:
            can_be_summed = self.task.__class__.logging_outputs_can_be_summed(self.get_criterion())
        if can_be_summed:
    ...
    def _reduce_and_log_stats(self, logging_outputs, sample_size, grad_norm=None, subset='train'):
        ...
        with metrics.aggregate() as agg:
            if logging_outputs is not None:
                function_signature = inspect.signature(self.task.reduce_metrics)
                if 'split' in function_signature.parameters:
                    self.task.reduce_metrics(logging_outputs, self.get_criterion(), split=subset)
                else:
